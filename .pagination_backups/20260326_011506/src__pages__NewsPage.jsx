              ...
              Object.entries(grouped).map(([monthYear, items]) => (
                <div key={monthYear} style={{ marginBottom:28 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
                    <div style={{ background:navy, color:gold, borderRadius:8, padding:'5px 16px', fontWeight:800, fontSize:12.5, whiteSpace:'nowrap', flexShrink:0 }}>📅 {monthYear}</div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                    {items.map(n => {
                      ...
                      return (
                        <div key={n.id} className="ntf-row-hover" style={{ background:'#fff', borderRadius:11, padding:'14px 18px', display:'flex', alignItems:'flex-start', gap:14, borderLeft:`4px solid ${tc.dot}`, border:`1px solid #edf2f7`, boxShadow:'0 2px 10px rgba(11,31,78,.04)' }}>
                          ...
                          <div style={{ flex:1, overflow:'hidden' }}>
                            ...
                            <div dangerouslySetInnerHTML={{ __html: clean(n.text) }} style={{ fontSize:14.5, color:'#334155', fontWeight:500, lineHeight:1.65 }} />
                            {n.link && (
                  ...
                  return (
                    <div key={n.id} className="ntf-card-hover" style={{ background:'#fff', borderRadius:13, overflow:'hidden', boxShadow:'0 4px 16px rgba(11,31,78,.07)', border:'1px solid #edf2f7' }}>
                      ...
                      <div style={{ padding:'15px 16px' }}>
                        <div dangerouslySetInnerHTML={{ __html: clean(n.text) }} style={{ fontSize:13.5, color:'#334155', lineHeight:1.7, marginBottom:12 }} />
                        {n.link && (
                          <a href={n.link} target="_blank" rel="noreferrer" className="ntf-dl-btn"
                             onClick={(e) => {
                               if(n.link.includes('drive.google') || n.link.endsWith('.pdf') || n.link.includes('firebase')) {
                                 e.preventDefault();
                                 setPreviewPdf({ url: n.link, title: n.type || 'News' });
