...
// Flash Popup + Red Banner — reads from Firestore 'alerts' collection
...
export default function AlertBanner() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, snap => {
      ...
      setAlerts(data);
      // Show popup for first undismissed alert
      const undismissed = data.filter(a => !dismissed.includes(a.id));
...
useEffect(() => {
  ...
  const dismissAlert = (id) => {
    ...
    setDismissed(next);
    sessionStorage.setItem('gnc_dismissed_alerts', JSON.stringify(next));
    const undismissed = alerts.filter(a => !next.includes(a.id));
    if (undismissed.length > 0) {
      setCurrentIdx(0);
    } else {
      setShowPopup(false);
  ...
  const bannerAlert = alerts[bannerIdx % Math.max(alerts.length, 1)];

  if (alerts.length === 0) return null;

  return (
    <>
      {/* ── Scrolling red banner (always visible for active alerts) ── */}
      <div style={{
        background: `linear-gradient(90deg, #dc2626, #b91c1c, #dc2626)`,
        color: '#fff', padding: '8px 0', overflow: 'hidden', position: 'relative',
        borderBottom: '2px solid #991b1b', zIndex: 9998,
      }}>
        <style>{`
        ...
        `}</style>
        <div className="gnc-alert-scroll" style={{ fontSize: 13.5, fontWeight: 700, letterSpacing: .3 }}>
          🚨&nbsp;&nbsp;
          {alerts.map((a, i) => (
            <span key={a.id}>
              {a.text}
              {i < alerts.length - 1 && <span style={{ margin: '0 32px', opacity: .5 }}>•</span>}
            </span>
          ))}
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </div>
          ...
          <div className="gnc-popup" style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ color: '#fff', fontWeight: 900, fontSize: 17, letterSpacing: -.3 }}>Urgent Notice</div>
                  <div style={{ color: 'rgba(255,255,255,.6)', fontSize: 12, marginTop: 2 }}>Guru Nanak College, Dhanbad</div>
                </div>
              </div>
              <button
            ...
            <div style={{ padding: '28px 30px' }}>
              <p style={{
                fontSize: 15.5, color: '#1e293b', lineHeight: 1.75, fontWeight: 500,
                margin: '0 0 24px',
              }}>
                {popupAlert.text}
              </p>
