      ...
      <div style={{ maxWidth: 1400, width: '100%', margin: '0 auto', padding: '0 15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '8px 0', flexShrink: 0 }}>
          <img src={`${import.meta.env.BASE_URL}images/logo1.png`} 
            ...
            style={{ height: isMobile ? 55 : 80, width: 'auto', objectFit: 'contain' }} 
          />
        </div>

        {isMobile && (
          <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'transparent', border: 'none', color: COLORS.navy, fontSize: 32, cursor: 'pointer', padding: '4px 8px', flexShrink: 0, zIndex: 200 }}>
            {menuOpen ? '✕' : '☰'}
          </button>
        )}
          ...
          {navLinks.map(l0 => (
            <div key={l0.label} style={{ position: 'relative', width: isMobile ? '100%' : 'auto' }}
              ...
              {l0.sub && openL1 === l0.label && (
                <div style={{ position: isMobile ? 'static' : 'absolute', top: '100%', left: 0, background: '#fff', minWidth: 260, boxShadow: isMobile ? 'none' : '0 12px 30px rgba(0,0,0,.15)', borderTop: isMobile ? 'none' : '3px solid ' + COLORS.navy, borderRadius: isMobile ? 8 : '0 0 8px 8px', zIndex: 200, padding: isMobile ? '5px 0' : '8px 0' }}>
                  {l0.sub.map(l1 => (
                    <div key={l1.label} style={{ position: 'relative' }} onMouseEnter={() => !isMobile && setOpenL2(l1.label)} onMouseLeave={() => !isMobile && setOpenL2(null)}>
                      ...
                      {l1.sub && openL2 === l1.label && (
                        <div style={{ position: isMobile ? 'static' : 'absolute', top: 0, left: '100%', background: '#fff', minWidth: 260, boxShadow: isMobile ? 'none' : '4px 4px 20px rgba(0,0,0,.15)', borderTop: isMobile ? 'none' : '3px solid ' + COLORS.gold, borderRadius: isMobile ? 4 : '0 8px 8px 8px', margin: isMobile ? '0 16px 10px' : 0, borderLeft: isMobile ? '2px solid ' + COLORS.gold : 'none' }}>
                          {l1.sub.map(l2 => (
