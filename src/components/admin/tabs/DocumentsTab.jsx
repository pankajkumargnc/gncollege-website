...
// ✅ BUG FIX: docData → formData, setDocData → setFormData, pdfLink → pdfUrl
...
const CATEGORIES = [
  ...
  'NAAC', 'Government', 'Committee', 'Other',
];
    ...
    <div className="fade-up">
      ...
      <div className="card-gold">
        ...
        <form onSubmit={save}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 14, marginBottom: 14 }}>
            <div style={{ gridColumn: '1/-1' }}>
              ...
              <input
                ...
                required
                placeholder="NAAC Self Study Report 2024-25"
              />
            </div>
            <div>
          ...
          {/* ✅ FIX: docData → formData, pdfLink → pdfUrl, setDocData → setFormData */}
          <div style={{ marginBottom: 20 }}>
            <MediaPicker
              label="Select Document PDF"
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
        {cats.map(c => (
          <button key={c} onClick={() => setCatFilter(c)} className="abtn abtn-sm"
            style={{
      ...
      <div className="card">
        ...
        {filtered.map(d => (
          <div key={d.id} className={`arow ${selected.includes(d.id) ? 'selected' : ''}`}>
            ...
            />
            <div style={{ width: 40, height: 40, borderRadius: 10, background: `${NAVY}12`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>
              📄
            </div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                ...
                {d.driveSource && <span className="abadge" style={{ background: '#e0f2fe', color: '#075985' }}>☁ Drive</span>}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              {(d.pdfUrl || d.link) && (
                <a href={d.pdfUrl || d.link} target="_blank" rel="noreferrer"
                  className="abtn abtn-outline abtn-sm" style={{ textDecoration: 'none' }}>
                  👁️ View
                </a>
              )}
