...
import { Zap, Sparkles, Send, Bell, Megaphone, Calendar, Radio, AlertTriangle, Edit2, Trash2, CheckCircle2, MessageSquare, Copy, ExternalLink, X, RefreshCw } from 'lucide-react';
        ...
        {items.length === 0 ? <div style={{ fontSize: 12, color: T.t4, textAlign: 'center', padding: '10px 0' }}>No items yet.</div> : items.map(item => (
          <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: `1px solid rgba(15,35,71,0.05)` }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              <button type="button" onClick={() => onEdit(item)} style={{ background: '#fff', border: '1.5px solid #e2e8f0', cursor: 'pointer', padding: '5px 8px', borderRadius: 6, transition: 'all 0.2s', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }} onMouseEnter={e => e.currentTarget.style.borderColor = accent} onMouseLeave={e => e.currentTarget.style.borderColor = '#e2e8f0'} aria-label="Edit item"><Edit2 size={12} color={NAVY} /></button>
              <button type="button" onClick={() => onDel(col, item.id, item.text || item.title)} style={{ background: '#fff', border: '1.5px solid #e2e8f0', cursor: 'pointer', padding: '5px 8px', borderRadius: 6, transition: 'all 0.2s', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }} onMouseEnter={e => e.currentTarget.style.borderColor = '#ef4444'} onMouseLeave={e => e.currentTarget.style.borderColor = '#e2e8f0'} aria-label="Delete item"><Trash2 size={12} color="#dc2626" /></button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="fade-up" style={{ padding: '2px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
        <div style={{ background: `${GOLD}20`, width: 56, height: 56, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: T.t3, fontSize: 14, fontWeight: 600, opacity: 0.8 }}>Rapid content management and visibility controls across live college surfaces.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 20, position: 'relative', zIndex: 10 }}>
        ...
        <div className="qp-card" style={{ background: '#f1f5f9', gridColumn: '1 / -1', display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: `1.5px solid ${NAVY}15`, flexWrap: 'wrap', gap: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, flex: '1 1 300px' }}>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: `${GOLD}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <button className="qp-btn-pub" onClick={() => setIsAiModalOpen(true)} style={{ background: NAVY, minWidth: 200, width: '100%', flex: '1 1 200px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <Sparkles size={16} /> Open Neural AI Studio
            </button>
        </div>
        ...
        <div className="qp-card" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 20, borderBottom: `1.5px solid rgba(15,35,71,0.06)`, paddingBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: NAVY, color: GOLD, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Radio size={24} />
              </div>
              <div>
            ...
            <span style={{ fontSize: 11, background: `${GOLD}18`, color: '#d97706', padding: '4px 12px', borderRadius: 20, fontWeight: 800, border: `1px solid ${GOLD}40`, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              <Zap size={13} /> Syncs 5 Channels Simultaneously
            </span>
          </div>
          ...
          <form onSubmit={handleMultiChannelDispatch}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, background: mcNotice ? `${NAVY}10` : '#f8fafc', padding: '8px 14px', borderRadius: 10, border: `1.5px solid ${mcNotice ? NAVY : '#e2e8f0'}`, cursor: 'pointer', fontSize: 12.5, fontWeight: 700, color: NAVY }}>
                  ...
                  <Bell size={14} /> Card 1: College Notice Board
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, background: mcAlert ? '#fee2e2' : '#f8fafc', padding: '8px 14px', borderRadius: 10, border: `1.5px solid ${mcAlert ? '#ef4444' : '#e2e8f0'}`, cursor: 'pointer', fontSize: 12.5, fontWeight: 700, color: '#b91c1c' }}>
                  ...
                  <AlertTriangle size={14} /> Flash Alert Modal Banner
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, background: mcTicker ? `${GOLD}18` : '#f8fafc', padding: '8px 14px', borderRadius: 10, border: `1.5px solid ${mcTicker ? GOLD : '#e2e8f0'}`, cursor: 'pointer', fontSize: 12.5, fontWeight: 700, color: '#b45309' }}>
                  ...
                  <Megaphone size={14} /> Card 2: News / Live Ticker
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, background: mcPush ? '#dcfce7' : '#f8fafc', padding: '8px 14px', borderRadius: 10, border: `1.5px solid ${mcPush ? '#22c55e' : '#e2e8f0'}`, cursor: 'pointer', fontSize: 12.5, fontWeight: 700, color: '#15803d' }}>
                  ...
                  <Radio size={14} /> Browser Push Notification
                </label>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
              <button 
                ...
                style={{ background: NAVY, padding: '12px 28px', fontSize: 14, minWidth: 240, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
              >
                <Send size={16} />
                <span>{mcSending ? 'Broadcasting...' : 'Broadcast Across Channels'}</span>
              </button>
            </div>
          </form>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button 
                    ...
                    style={{ background: '#15803d', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 800, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}
                  >
                  ...
                  <a 
                    ...
                    style={{ background: '#25D366', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 800, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6 }}
                  >
      ...
      {isAiModalOpen && (
        ...
        <div className={`qp-card ${notData.id ? 'active' : ''}`}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div className="qp-icon-box" style={{ background: '#eff6ff', color: '#3b82f6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Bell size={20} />
              </div>
        ...
        <div className={`qp-card ${newsData.id ? 'active' : ''}`}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div className="qp-icon-box" style={{ background: '#f3e8ff', color: '#8b5cf6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Megaphone size={20} />
              </div>
          ...
          <form onSubmit={saveNews} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button type="submit" className="qp-btn-pub" style={{ flex: 1, background: '#8b5cf6' }} disabled={loading}>
                {loading ? 'Saving…' : newsData.id ? 'Update News' : 'Publish News'}
              </button>
        ...
        <div className={`qp-card ${evtData.id ? 'active' : ''}`}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div className="qp-icon-box" style={{ background: '#dcfce7', color: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Calendar size={20} />
              </div>
          ...
          <form onSubmit={saveEvent} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
              <button type="submit" className="qp-btn-pub" style={{ flex: 1, background: '#10b981' }} disabled={loading}>
                {loading ? 'Saving…' : evtData.id ? 'Update Event' : 'Schedule Event'}
              </button>
        ...
        <div className={`qp-card ${updData.id ? 'active' : ''}`}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
            <div className="qp-icon-box" style={{ background: '#fef3c7', color: '#d97706', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Radio size={20} />
            </div>
          ...
          <form onSubmit={saveUpdate} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button type="submit" className="qp-btn-pub" style={{ flex: 1, background: '#d97706' }} disabled={loading}>
                {loading ? 'Saving…' : updData.id ? 'Update Ticker' : 'Post to Ticker'}
              </button>
        ...
        <div className={`qp-card ${altData.id ? 'active' : ''}`} style={{ background: '#fffafa' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
            <div className={`qp-icon-box ${!altData.id ? 'pulse-red' : ''}`} style={{ background: '#ef4444', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <AlertTriangle size={20} />
            </div>
            <div className="qp-label" style={{ color: '#ef4444' }}>{altData.id ? 'Edit Emergency Alert' : 'Flash Alert (Emergency)'}</div>
          </div>
          <form onSubmit={saveAlert} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button type="submit" className="qp-btn-pub" style={{ flex: 1, background: '#ef4444' }} disabled={loading}>
                {loading ? 'Saving…' : altData.id ? 'Update Alert' : 'Broadcast Alert'}
              </button>
