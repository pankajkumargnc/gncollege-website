...
// 🌟 FIX 1: Prop ka naam wapas 'slides' kar diya gaya hai (HomePage se match karne ke liye)
const HeroSlider = ({ slides = [] }) => {
  const [currentSlide, setCurrentSlide] = useState(0);

  // Fallback images agar Firebase mein koi slide nahi hai
  const FALLBACK = [
  ...
  // 🌟 FIX 2: Data ko smart tarike se sort karna taaki baar-baar render na ho
  const displaySlides = useMemo(() => {
    if (!slides || slides.length === 0) return FALLBACK;
    // Order ke hisaab se sort karna
    return [...slides].sort((a, b) => (Number(a.order) || 0) - (Number(b.order) || 0));
  }, [slides]);

  const slideLength = displaySlides.length;

  // 🌟 FIX 3: Bulletproof Auto-Scroll Logic (Ab stuck nahi hoga)
  useEffect(() => {
    if (slideLength <= 1) return; // Ek photo par rotate nahi karega
  ...
  // Agar slide delete/add ho, toh wapas first photo par aa jaye
  useEffect(() => {
    setCurrentSlide(0);
  }, [slideLength]);
  ...
  const resolveImage = (src) => {
    if (!src) return '';
    // Agar external API (ImgBB) ka link hai toh direct return karega
    if (src.startsWith('http://') || src.startsWith('https://')) return src; 
    // Agar local photo hai toh public folder se uthayega
  ...
  return (
    <div className="slider">
      {/* Controls - Agar 1 se zyada photo hongi tabhi arrows dikhenge */}
      {slideLength > 1 && (
        <>
