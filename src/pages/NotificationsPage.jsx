...
const TYPE_COLORS = {
  ...
  Examination: { bg:'#FFF5F5', text:'#742a2a', border:'#FEB2B2', dot:'#e53e3e' },
  Admission:   { bg:'#F0FFF4', text:'#1c4532', border:'#9AE6B4', dot:'#38a169' },
            ...
            <section style={{ background: '#fff', padding: '30px 40px', borderRadius: '16px', boxShadow: '0 8px 25px rgba(0,0,0,0.07)', animationDelay:'.1s' }}>
              <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'center', marginBottom:16 }}>
                <div style={{ flex:1, minWidth:200, position:'relative' }}>
                  ...
                  <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Notice search karo..."
            ...
            <section style={{ background: '#fff', padding: '30px 40px', borderRadius: '16px', boxShadow: '0 8px 25px rgba(0,0,0,0.07)', marginTop: '30px', animationDelay:'.2s' }}>
              ...
              {loading ? (
                <div style={{ textAlign:'center', padding:'60px 20px' }}>
                  ...
                  <p style={{ color:'#718096', fontWeight:600 }}>Notices load ho rahi hain...</p>
                </div>
              ) : filtered.length === 0 ? (
                <div style={{ textAlign:'center', padding:'50px 20px' }}>
                  <div style={{ fontSize:44, marginBottom:10 }}>🔍</div>
                  <h3 style={{ color:navy, fontWeight:800, margin:'0 0 6px' }}>Koi notice nahi mili</h3>
                ...
                Object.entries(grouped).map(([monthYear, items]) => (
                  <div key={monthYear} style={{ marginBottom:28 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
                      ...
                      <span style={{ fontSize:11.5, color:'#a0aec0', fontWeight:700, flexShrink:0 }}>{items.length} notice{items.length>1?'s':''}</span>
                    </div>
                          ...
                          <div key={n.id} className="ntf-row-hover"
                            ...
                            <div style={{ flex:1, overflow:'hidden' }}>
                              <div style={{ display:'flex', gap:7, marginBottom:7, flexWrap:'wrap', alignItems:'center' }}>
                                <span style={{ background:tc.bg, color:tc.text, border:`1px solid ${tc.border}`, padding:'2px 9px', borderRadius:20, fontSize:11, fontWeight:700 }}>{n.type||'General'}</span>
                                {n.isNew && <span style={{ background:'#e53e3e', color:'#fff', padding:'2px 7px', borderRadius:20, fontSize:9.5, fontWeight:900, animation:'blink 1.5s infinite' }}>● NEW</span>}
                              </div>
                    ...
                    return (
                      <div key={n.id} className="ntf-card-hover"
                        ...
                        <div style={{ padding:'15px 16px' }}>
                          ...
                          {n.link && (
                            <a href={n.link} target="_blank" rel="noreferrer" className="download-btn">📎 Attachment</a>
                          )}
                        </div>
                      </div>
                    );
