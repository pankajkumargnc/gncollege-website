...
// ✅ FIXED: DOMPurify added on both list + card views; search also checks type field
...
import DOMPurify from 'dompurify'; // ✅ XSS fix
...
const clean   = html => DOMPurify.sanitize(html || ''); // ✅ single helper
...
export default function NewsPage() {
  ...
  const filtered = useMemo(() => newsList.filter(n => {
    ...
    // ✅ FIX: search mein type bhi check hota hai
    if (search) {
  ...
  return (
    <div className="profile-page-wrapper">
                      ...
                      return (
                        <div key={n.id} className="ntf-row-hover"
                          ...
                          <div style={{ flex:1, overflow:'hidden' }}>
                            ...
                            {/* ✅ DOMPurify sanitize */}
                            <div dangerouslySetInnerHTML={{ __html: clean(n.text) }}
                ...
                {filtered.map(n => {
                  ...
                  return (
                    <div key={n.id} className="ntf-card-hover"
                      style={{ background:'#fff', borderRadius:13, overflow:'hidden', boxShadow:'0 4px 16px rgba(11,31,78,.07)', border:'1px solid #edf2f7' }}>
                      ...
                      <div style={{ padding:'15px 16px' }}>
                        ...
                        {n.link && (
                          <a href={n.link} target="_blank" rel="noreferrer" className="ntf-dl-btn">🔗 Read More</a>
                        )}
                      </div>
                    </div>
                  );
      ...
      <style>{`
        .ntf-dl-btn { display:inline-block; background:#f8fafc; color:${navy}; padding:8px 15px; border-radius:6px; font-size:12px; font-weight:700; text-decoration:none; border:1px solid #cbd5e1; transition:.2s; }
