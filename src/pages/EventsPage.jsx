...
export default function EventsPage() {
  ...
  // ✅ SMART URL RESOLVER FOR GITHUB PAGES
      ...
      <div style={{ maxWidth: '1200px', margin: '-50px auto 0', padding: '0 20px', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ display:'flex', gap:16, flexWrap:'wrap', justifyContent: 'center', marginBottom: 40 }}>
          {[
            ...
            <div key={i} style={{ 
              background: '#fff', border: `1px solid ${s.hi ? gold : '#e2e8f0'}`, borderRadius: 14, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 8px 20px rgba(0,0,0,0.06)', flex: 1, minWidth: '200px' 
            }}>
              ...
              <div>
                <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 20, fontWeight: 800, color: s.hi ? '#d97706' : navy, lineHeight: 1.1 }}>{s.val}</div>
        ...
        <main>
            ...
            <section style={{ background: '#fff', padding: '30px 40px', borderRadius: '16px', boxShadow: '0 8px 25px rgba(0,0,0,0.07)', marginTop: '30px' }}>
              <div style={{ display:'flex', gap:3, marginBottom:16, background:'#f4f7fa', borderRadius:11, padding:3, width:'fit-content' }}>
                {[
                  ...
                  <button key={t.id} className="evt-fb" onClick={() => setTab(t.id)} style={{ padding:'8px 18px', borderRadius:9, background:tab===t.id?navy:'transparent', color:tab===t.id?'#fff':'#718096', fontWeight:700, fontSize:13, display:'flex', alignItems:'center', gap:7 }}>
                    {t.label} <span style={{ background:tab===t.id?gold:'#e2e8f0', color:tab===t.id?navy:'#718096', borderRadius:20, padding:'1px 8px', fontSize:11.5, fontWeight:800 }}>{t.count}</span>
                  </button>
                ))}
              </div>
              <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'center', marginBottom:12 }}>
                <div style={{ flex:1, minWidth:200, position:'relative' }}>
                  ...
                  <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Event search karo..." style={{ width:'100%', padding:'10px 14px 10px 38px', border:'2px solid #e2e8f0', borderRadius:10, fontSize:14, fontFamily:'inherit', background:'#f8fafc', outline:'none', boxSizing:'border-box' }} />
                </div>
            ...
            <section style={{ background: '#fff', padding: '30px 40px', borderRadius: '16px', boxShadow: '0 8px 25px rgba(0,0,0,0.07)', marginTop: '30px', marginBottom: '60px' }}>
              <h2 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 'clamp(24px, 4vw, 32px)', fontWeight: 800, color: navy, margin: '0 0 10px', letterSpacing: '-0.5px' }}>
              ...
              {loading ? (
                ...
                <div style={{ textAlign:'center', padding:'50px 20px' }}>🎭 Koi event nahi mila</div>
              ) : (
                Object.entries(grouped).map(([month, items]) => (
                  <div key={month} style={{ marginBottom:40 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
                      <div style={{ background:`linear-gradient(135deg,${navy},#1a3a7c)`, color:gold, borderRadius:9, padding:'6px 18px', fontWeight:900, fontSize:13.5, boxShadow:`0 4px 14px ${navy}22` }}>📅 {month}</div>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:24 }}>
                      {items.map(ev => {
                        ...
                        const imgSrc = resolveUrl(ev.displayImage); // ✅ GitHub Fix Applied

                        return (
                          <div key={ev.id} className={`evt-card${isUp?' evt-upcoming':''}`} style={{ background:'#fff', borderRadius:16, overflow:'hidden', boxShadow:'0 4px 16px rgba(11,31,78,.06)', border:`1px solid ${isUp?gold:'#edf2f7'}` }}>

                            {/* Image Section - Fixed Height */}
                            {imgSrc ? (
                              <div style={{ height:180, position:'relative', overflow:'hidden', flexShrink: 0 }}>
                                <img src={imgSrc} alt={ev.title} className="evt-img" style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={e => { e.target.parentElement.style.background=m.grad; e.target.style.display='none'; }} />
                              ...
                              <div style={{ height:180, background:m.grad, padding:'20px', flexShrink: 0 }} />
                            )}

                            {/* Content Section - Flex Grow */}
                            <div style={{ padding:'20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                              <h3 style={{ margin:'0 0 8px', fontSize:16, fontWeight:800, color:navy, lineHeight:1.3 }}>{ev.title}</h3>
                              <p style={{ margin:'0 0 12px', fontSize:12.5, color:'#718096', fontWeight: 600 }}>📍 {ev.venue||ev.location||'College Campus'}</p>
                              
                              {plainText.length > 0 && (
                                <div style={{ marginBottom: '15px' }}>
                                  <p style={{ margin:0, fontSize:13, color:'#4a5568', lineHeight:1.6, textAlign: 'justify' }}>
                                    {exp ? plainText : plainText.substring(0, 120) + (plainText.length > 120 ? '…' : '')}
                                  </p>
                                  {plainText.length > 120 && (
                                    <button onClick={() => setExpandId(exp ? null : ev.id)} style={{ background:'none', border:'none', color:gold, fontWeight:800, fontSize:12, cursor:'pointer', padding:'4px 0 0' }}>
                              ...
                              {/* Actions - Pushed to Bottom Perfectly */}
                              <div style={{ marginTop: 'auto', paddingTop: '15px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ background:m.light, color:m.text, padding:'4px 10px', borderRadius:20, fontSize:11, fontWeight:700 }}>{ev.type}</span>
                                {(ev.reportLink || ev.pdfLink) && (
                                  <button className="download-btn" onClick={() => setPreviewPdf({ url: resolveUrl(ev.reportLink || ev.pdfLink), title: ev.title })}>
                                    📄 View Report
                                  </button>
                                )}
                              </div>
