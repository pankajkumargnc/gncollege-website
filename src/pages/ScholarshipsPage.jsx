...
import {
  BookOpen, Target, GraduationCap, Award, Landmark, CheckCircle2,
  ClipboardList, Laptop, Building2, BadgeIndianRupee, Phone, Mail, Star
} from 'lucide-react';

const SCHOLARSHIPS = [
  {
    title: 'National Scholarship Portal (NSP)',
    icon: <Landmark size={24} style={{ color: '#0f2347' }} />,
  ...
  },
  {
    ...
    icon: <BookOpen size={24} style={{ color: '#0f2347' }} />,
  ...
  },
  {
    ...
    icon: <Target size={24} style={{ color: '#0f2347' }} />,
    provider: 'Ministry of Social Justice',
  ...
  },
  {
    ...
    icon: <span style={{ fontSize: 24, fontWeight: 'bold', color: '#0f2347' }}>☬</span>,
  ...
  },
  {
    ...
    icon: <GraduationCap size={24} style={{ color: '#0f2347' }} />,
  ...
  },
  {
    ...
    icon: <Award size={24} style={{ color: '#f4a023' }} />,
...
const STEPS = [
  { step: 1, title: 'Check Eligibility', desc: 'Verify your eligibility on the NSP portal or consult the college administrative office.', icon: <CheckCircle2 size={20} style={{ color: '#10b981' }} /> },
  { step: 2, title: 'Gather Documents', desc: 'Prepare your Caste/Income certificate, Aadhaar card, marksheets, and bank passbook.', icon: <ClipboardList size={20} style={{ color: '#0284c7' }} /> },
  { step: 3, title: 'Apply Online', desc: 'Register yourself on the official NSP portal (scholarships.gov.in).', icon: <Laptop size={20} style={{ color: '#8b5cf6' }} /> },
  { step: 4, title: 'Institute Verification', desc: 'Submit your online application number to the college office for verification.', icon: <Building2 size={20} style={{ color: '#f59e0b' }} /> },
  { step: 5, title: 'DBT Transfer', desc: 'The scholarship amount is credited to your bank account after final government approval.', icon: <BadgeIndianRupee size={20} style={{ color: '#10b981' }} /> },
];
    ...
    <div className="profile-page-wrapper" style={{ minHeight: '80vh' }}>
      ...
      <section className="premium-hero">
        ...
        <div className="hero-content-wrapper anim-fade-in">
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(244, 160, 35, 0.18)', border: '1px solid rgba(244, 160, 35, 0.4)', borderRadius: 20, padding: '4px 14px', marginBottom: 14 }}>
            <span style={{ fontSize: 13, color: '#f4a023', fontWeight: 800, letterSpacing: 0.5 }}>STUDENT WELFARE &amp; FINANCIAL SUPPORT</span>
          </div>
          <h1 className="hero-title" style={{ margin: '0 0 10px' }}>
            {splitHeading('Scholarships & Financial Aid')}
          </h1>
        ...
        <div style={{
          ...
          {SCHOLARSHIPS.map((s, idx) => (
            <div key={idx} className="sch-card" style={{
              ...
              {s.highlight && (
                <div style={{ background: '#f4a023', color: '#0f2347', padding: '6px 16px', fontSize: 11, fontWeight: 900, textAlign: 'center', textTransform: 'uppercase', letterSpacing: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  <Star size={12} fill="#0f2347" /> Recommended for GNC Students
                </div>
              )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {STEPS.map(s => (
              <div key={s.step} className="sch-step" style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 800, color: '#0f2347', fontSize: 'clamp(0.9rem, 1.5vw, 1.05rem)', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <section style={{
        ...
        <div className="sch-helpdesk" style={{
          ...
          <h3 style={{ fontSize: 'clamp(1.1rem, 2vw, 1.3rem)', fontWeight: 800, color: '#0f2347', marginBottom: 12, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            <Building2 size={22} style={{ color: '#f4a023' }} /> Scholarship Helpdesk
          </h3>
