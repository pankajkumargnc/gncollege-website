...
// YouTube Data API v3 — Auto-fetches latest videos from college channel
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      const plRes = await fetch(
        `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${uploadId}&maxResults=${maxResults}&key=${apiKey}`
      );
      ...
      // Step 3: Get full video details (views, duration, likes)
        ...
        <div className="hero-content anim-fade-in">
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 22 }}>
            {[
              ...
              { val: videos.filter(v => v.statistics?.likeCount > 0).length, label: 'With Likes' },
            ].map((s, i) => (
        ...
        {!ytConfig?.apiKey ? (
          <div style={{ textAlign: 'center', padding: '80px 20px', background: '#fff', borderRadius: 20, boxShadow: '0 8px 30px rgba(11,31,78,.07)' }}>
            ...
            <p style={{ color: '#64748b', fontSize: 15 }}>
              Admin Panel mein जाएं → YouTube Manager tab → API Key aur Channel ID add karein.
            </p>
          </div>
        ) : loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px', background: '#fff5f5', borderRadius: 16, border: '1px solid #fed7d7' }}>
            ...
            <button onClick={fetchVideos}
              style={{ marginTop: 16, padding: '10px 24px', background: navy, color: '#fff', border: 'none', borderRadius: 10, fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>
              🔄 Retry
            </button>
          </div>
        ) : (
                  ...
                  <div key={vid} className="yt-card" style={{ animationDelay: `${i * .06}s` }}>
                    <div className="yt-thumb" onClick={() => setPlayingId(isPlaying ? null : vid)} style={{ cursor: 'pointer' }}>
                      {isPlaying ? (
                        ...
                        <>
                          <img
                            src={sn.thumbnails?.maxres?.url || sn.thumbnails?.high?.url || sn.thumbnails?.medium?.url}
                            alt={sn.title}
                          />
                    ...
                    <div style={{ padding: '16px 18px 18px' }}>
                      ...
                      <p style={{ margin: '0 0 12px', fontSize: 12, color: '#a0aec0', fontWeight: 600 }}>
                        📅 {fmtDate(sn.publishedAt)}
                      </p>
