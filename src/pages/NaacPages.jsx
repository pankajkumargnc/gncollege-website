...
const PageHeader = ({ title, subtitle, icon }) => (
  <div style={{ background: NAVY, padding: '80px 20px 60px', textAlign: 'center', color: '#fff' }}>
    <Fade>
      ...
      <p style={{ color: '#cbd5e1', fontSize: 16, maxWidth: 700, margin: '0 auto', lineHeight: 1.6 }}>{subtitle}</p>
    </Fade>
  </div>
);

/* ─── NAYA: Document List Component (Used for AQAR, NIRF, SSR) ─── */
      ...
      {docs.length === 0 ? (
        ...
        docs.map((d, i) => (
          <Fade key={d.id} delay={i * 0.05}>
            <a href={d.link} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 20, background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', textDecoration: 'none', transition: '0.2s', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }} onMouseEnter={e=>{e.currentTarget.style.borderColor=GOLD; e.currentTarget.style.transform='translateY(-3px)'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='#e2e8f0'; e.currentTarget.style.transform='none'}}>
              <div style={{ fontSize: 32 }}>📄</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, color: NAVY, fontSize: 15, lineHeight: 1.3 }}>{d.title}</div>
                <div style={{ fontSize: 12, color: GOLD, fontWeight: 700, marginTop: 4, textTransform: 'uppercase' }}>Download PDF ↗</div>
              </div>
            </a>
          </Fade>
        ))
...
export function SsrCyclePage({ cycle = 1 }) {
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif" }}>
      <PageHeader title={title} subtitle={`Complete Self Study Report and Peer Team documents for NAAC Accreditation Cycle ${cycle}.`} icon={icon} />
...
/* ════════════════════════════════════════════════════════════
   2. AQAR (Annual Quality Assurance Report)
════════════════════════════════════════════════════════════ */
export function AqarPage() {
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: '-40px auto 80px', padding: '0 20px', position: 'relative', zIndex: 10 }}>
        <NaacDocumentList categoryKey="aqar" emptyMsg="Annual Quality Assurance Reports will appear here." />
      </div>
    </div>
  );
}
...
export function NirfPage() {
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: '-40px auto 80px', padding: '0 20px', position: 'relative', zIndex: 10 }}>
        <NaacDocumentList categoryKey="nirf" emptyMsg="NIRF participation data and reports will be updated here." />
      </div>
    </div>
  );
}
